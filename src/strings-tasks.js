...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replaceAll(/\W/g, '');
...
function reverseWords(str) {
  ...
  const reversedWordsArr = arr.map((item) => {
...
function encodeToRot13(str) {
  const strAM = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const strNZ = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  const arrOfStr = str.split('');

  for (let i = 0; i < arrOfStr.length; i += 1) {
    const index = strAM.indexOf(arrOfStr[i]);
    if (index < 0) newStr += arrOfStr[i];
    else newStr += strNZ[index];
  }
